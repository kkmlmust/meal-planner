...
@mcp.tool()
async def save_recipe(telegram_id: str, title: str, ingredients: list[str], instructions: str, prep_time: int) -> str:
    """Save a newly generated recipe to the database.
    Use this AFTER the LLM generates a recipe that isn't already in the database.
    This ensures the recipe is saved for future lookups instead of regenerating.
    Parameters:
    - title: recipe title (e.g., 'Chicken Stir Fry')
    - ingredients: list of ingredient names (e.g., ['chicken', 'rice', 'soy sauce'])
    - instructions: step-by-step cooking instructions
    - prep_time: estimated cooking time in minutes
    Returns confirmation that the recipe was saved.
    """
    try:
        result = await api_client.save_recipe(
            title=title,
        ...
        return f"✅ Recipe saved: **{result['title']}** (ID: {result['id']}) — {len(ingredients)} ingredients linked in database"
    except Exception as e:
        return f"⚠️ Failed to save recipe: {e}"
...
async def get_cooking_history(telegram_id: str, days: int = 7) -> str:
    ...
    for log in logs:
        ...
        lines.append(f"#{log['id']} {log['recipe_title']} ({cooked_date}){notes_text}")
    return f"Your recent meals (last {days} days):\n" + "\n".join(lines)


@mcp.tool()
async def clear_ingredients(telegram_id: str) -> str:
    """Delete ALL ingredients from the user's pantry.
    Use this ONLY when the user explicitly asks to clear/remove all ingredients.
    This is irreversible — confirm with the user before deleting.
    Returns confirmation of how many items were cleared.
    """
    # First get the current ingredients to show count
    current = await api_client.get_ingredients(telegram_id)
    count = len(current)
    if count == 0:
        return "Your pantry is already empty! 🧹"
    await api_client.clear_ingredients(telegram_id)
    return f"🗑️ Cleared all {count} ingredient(s) from your pantry!"


@mcp.tool()
async def delete_cooking_log(telegram_id: str, log_id: int) -> str:
    """Delete a single cooking log entry by its ID.
    Use this when the user wants to remove one specific entry from their cooking history
    (e.g., if they accidentally logged something).
    The log ID is shown in the cooking history list (e.g., #12 Pasta Bolognese).
    Returns confirmation of deletion.
    """
    try:
        result = await api_client.delete_cooking_log(telegram_id, log_id)
        return f"✅ Deleted log entry #{log_id} from your cooking history."
    except Exception as e:
        return f"⚠️ Failed to delete log #{log_id}: {e}"
