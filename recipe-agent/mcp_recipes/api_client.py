import httpx
from mcp_recipes.config import settings
...
class RecipeAPIClient:
    ...
    async def clear_ingredients(self, telegram_id: str) -> dict:
        return await self._request("DELETE", f"/ingredients/{telegram_id}")
    ...
    async def save_recipe(
        self, title: str, ingredients: list[str], instructions: str, prep_time: int, source: str = "llm"
    ) -> dict:
    ...
    async def delete_cooking_log(self, telegram_id: str, log_id: int) -> dict:
        return await self._request(
            "DELETE", f"/cooking-log/{telegram_id}/{log_id}"
        )
